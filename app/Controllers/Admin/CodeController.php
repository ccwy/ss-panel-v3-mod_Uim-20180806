...
class CodeController extends AdminController
{
    ...
    {
        $table_config['total_column'] = array("id" => "ID", "usedatetime" => "使用时间", "number" => "操作", "userid" => "用户ID",
                        "user_name" => "用户名", "code" => "内容", "type" => "类型", "isused" => "是否已经使用");
    ...
    public function ajax_code($request, $response, $args)
    {
        ...
        $datatables->query('Select code.id,code.code,code.type,code.number,code.isused,code.userid,code.userid,user.user_name,code.usedatetime from code as userid,user as id WHERE code.userid=user.id');
